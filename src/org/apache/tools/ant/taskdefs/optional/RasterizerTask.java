...
public class RasterizerTask extends MatchingTask {
    ...
    public RasterizerTask() {
        this.converter = new SVGConverter(new RasterizerTaskSVGConverterController(this));
    }
     ...
     */
    public void setResult(final ValidImageTypes type) {
     ...
     * @param height Attribute value.
     */
    public void setMaxheight(final float height) {
     ...
     */
    public void setMedia(final ValidMediaTypes media) {
     ...
     * The attribute is optional.</p>
     *
     * @param language Attribute value.
     */
    ...
    public void execute() throws BuildException {
        ...
        final String defaultParser = XMLResourceDescriptor.getXMLParserClassName();
        // Throws BuildException.
        XMLResourceDescriptor.setXMLParserClassName(getParserClassName(this.readerClassName));
        ...
        try {
            ...
            this.converter.setSources(sources);

            // Set destination.
            if(this.srcFile != null) {
                this.converter.setDst(this.destFile);
            } else {
                this.converter.setDst(this.destDir);
            }
            ...
            } catch(final SVGConverterException sce) {
    ...
    throws BuildException {
        if(this.resultType != null) {
            this.converter.setDestinationType(this.resultType);
        } else {
        ...
        if(!Float.isNaN(this.maxWidth)) {
            ...
            this.converter.setMaxWidth(this.maxWidth);
        }
        if(!Float.isNaN(this.maxHeight)) {
            if(this.maxHeight < 0) {
                throw new BuildException("Value of maxheight parameter must positive.");
            }
            this.converter.setMaxHeight(this.maxHeight);
        }
        // The quality is just swallowed if the result type is not correct.
        if(allowedToSetQuality(this.resultType)) {
            if(!Float.isNaN(this.quality)) {
                // Throws BuildException.
                this.converter.setQuality(getQuality(this.quality));
            } else {
                // Set something to quiet irritating error
                // from JPEGTranscoder.
                this.converter.setQuality(DEFAULT_QUALITY);
            }
        }
        if(this.area != null) {
            // Throws BuildException.
            this.converter.setArea(getAreaOfInterest(this.area));
        }
        if(this.background != null) {
            // Throws BuildException.
            this.converter.setBackgroundColor(getBackgroundColor(this.background));
        }
        if(this.mediaType != null) {
            // Ant takes care of the correct media type values.
            this.converter.setMediaType(this.mediaType);
        }
        if(!Float.isNaN(this.dpi)) {
            ...
            this.converter.setPixelUnitToMillimeter(25.4f/this.dpi);
        }
    ...
    protected String[] getSourceFiles() {
        ...
        final List inputFiles = new ArrayList(); // Input files in temp list.
        ...
        if(this.srcFile != null) {
            ...
            if(this.srcDir != null) {
                ...
                final DirectoryScanner ds = this.fileset.getDirectoryScanner(this.project);
                final String[] includedFiles = ds.getIncludedFiles();
                // Add file and its path to the input file vector.
                for (int j = 0 ; j < includedFiles.length ; j++) {
                    final File newFile = new File(this.srcDir.getPath(), includedFiles[j]);
            ...
            for (int i = 0 ; i < this.filesets.size() ; i++) {
                ...
                // should be in input files.
                final FileSet fs = (FileSet) this.filesets.elementAt(i);
                final DirectoryScanner ds = fs.getDirectoryScanner(this.project);
                final String[] includedFiles = ds.getIncludedFiles();
                // Add file and its path to the input file vector.
                for (int j = 0 ; j < includedFiles.length ; j++) {
                    final File newFile = new File(fs.getDir(this.project).getPath(), includedFiles[j]);
                    inputFiles.add(newFile.getAbsolutePath());
                }
            }
        }
     ...
     */
    protected DestinationType getResultType(final String type) {
     ...
     */
    protected boolean allowedToSetQuality(final DestinationType type) {
     ...
     */
    protected float getQuality(final float quality)
    throws BuildException {
     ...
     */
    protected Rectangle2D getAreaOfInterest(final String area)
    throws BuildException {
        ...
        final String token;       // A token from the input string.
        final StringTokenizer tokenizer = new StringTokenizer(area, ", \t\n\r\f");
     ...
     */
    protected Color getBackgroundColor(final String argb)
    throws BuildException {
        ...
        final String token;       // A token from the input string.
        final StringTokenizer tokenizer = new StringTokenizer(argb, ", \t\n\r\f");
        // Input string tokenizer.
    ...
    private String getParserClassName(final String className) {
        ...
        System.err.println("[RasterizerTask] getParserClassName - " + className);
        if (name == null) {
            name = JAXP_PARSER;
        }
        if(name.equals(JAXP_PARSER)) {
            // Set first JAXP parser.
            // Throws BuildException.
            final XMLReader reader = JAXPUtils.getXMLReader();
